...
ggboxplot(df_new , x = "int_groups" , y = "trestbps" , title =  "Box Plot Graph")
...
# cp has meanfull differences which that means cp is effecting trestbps 

# comprising groups with each other
TukeyHSD(anova_1)
#  p adjs too high it's not meanful

tukey_hsd(anova_1) # with this way we can only see 10 row
a <- tukey_hsd(anova_1)
View(a)
